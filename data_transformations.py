...
class Transformations:
    ...
    def remove_rows_without_atsymbol_email_address(df):
        """Returns filtered dataframe: drops rows with no @ in email address."""
        return df[df["email_address"].str.contains("@")]


    def clean_user_data_rows_all_NULL(df):
        """
        Returns the transformed dataframe: drops rows consisting entirely of the string 'NULL' as entries.
        """
    ...
    def clean_upper_or_numeric_rows(df):
        """
        Returns transformed dataframe: drops rows consisting entirely of upper or numeric strings from the input.
        """
        return df.drop(df[df.map(Transformations.is_upper_or_numeric).all(axis=1)].index)


    def is_upper_or_numeric(val):
        """
        Checks if a string is uppercase or numeric.
        """
